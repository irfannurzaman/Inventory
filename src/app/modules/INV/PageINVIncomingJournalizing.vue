  ...
  setup(_, { root: { $api } }) {
    ...
    const FETCH_DATA = async (api, body?) => {
      ...
      if (api == 'glLinkstockPrepare') {
        state.searches = await dataIncomingJournalizing(GET_DATA, 'prepare')
    ...
    const onSearch = (val) => {
      value = val
      if (val.searches.date == null || val.searches.referenceNumber == '') {
        NotifyCreate('Unfilled field(s) detected', 'red', 'top')
      } else {
        dataIncomingJournalizing(val, 'keySearch')
        FETCH_DATA('glLinkstockBtnGo', paramsIncomingJournalizing())
